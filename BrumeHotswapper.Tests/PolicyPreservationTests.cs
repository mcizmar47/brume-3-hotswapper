...
public class PolicyPreservationTests
{
    ...
    public async Task Ipv6IsAnIndependentProductionBlock(string setting)
    {
        ...
        var checks=new List<BrumeHotswapper.Preflight.Check>();
        await BrumeHotswapper.Preflight.RoutingDiagnostics.VerifyAsync(router,"vpn",checks.Add,default);
        Assert.Contains(checks,c=>c.Name=="Kill-switch configuration"&&c.Status=="PASS");
        Assert.Contains(checks,c=>c.Name=="IPv6 compatibility"&&c.Status=="BLOCK");
    }
