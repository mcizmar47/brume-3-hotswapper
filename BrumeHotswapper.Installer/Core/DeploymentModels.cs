...
public record LanInventory(LanNetwork Network, IReadOnlyList<LanClient> Clients, IReadOnlyList<Reservation> Reservations);
...
public static class DeploymentPlanning
{
    ...
    public static InstallationPlan Create(InstallerConfiguration config, RouterSnapshot snapshot)
    {
        ...
        var reservations = DhcpPlanner.Plan(config.Guards, snapshot.Lan.Reservations, snapshot.Lan.Clients, snapshot.Lan.Network.Address);
        foreach (var r in reservations)
            if (!snapshot.Lan.Network.ContainsHost(r.Ip)) throw new SafeFailure("A protected device address is outside the usable LAN subnet. Refresh LAN discovery.");
