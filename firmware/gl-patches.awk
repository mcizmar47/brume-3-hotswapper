...
BEGIN {
    ...
    if (target == "instances") {
        ...
        anchor[2] = "local function main()"
        replacement[2] = "local function main()\n    -- Owned reconciliation enters through locked rtp2, which disarms first.\n    if os.getenv(\"HS_LOCKED\") ~= \"1\" and os.execute(\"/root/hotswapper/gl-coordination.sh daemon\") == 0 then return end"
        count = 2
    }
    if (target == "setup") {
        anchor[1] = "#!/bin/sh"
